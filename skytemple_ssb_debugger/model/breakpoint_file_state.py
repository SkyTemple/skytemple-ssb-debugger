...
class BreakpointFileState:
    ...
    def __init__(
        self, ssb_filename: str, opcode_addr: int, parent: BreakpointState | None
    ):
    ...
    def process(
        self,
        loaded_ssb: SsbLoadedFile,
        opcode_offset: int,
        project_fm: ProjectFileManager,
    ):
    ...
    def _make_epxs_absolute(self, project_fm, ssb_rom_filename, relpath_included_file):
        abs_path_ssb_exps = (
            project_fm.dir()
            + os.path.sep
            + project_fm.explorerscript_get_path_for_ssb(ssb_rom_filename)
        )
        return os.path.abspath(
            os.path.join(os.path.dirname(abs_path_ssb_exps), relpath_included_file)
        )
