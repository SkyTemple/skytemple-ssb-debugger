...
from skytemple_files.common.project_file_manager import (
    EXPLORERSCRIPT_INCLUSION_MAP_SUFFIX,
)
...
class ExpsMacroFileScriptFileContext(AbstractScriptFileContext):
    ...
    def __init__(
        self,
        absolute_path: str,
        ssb_fm: SsbFileManager,
        editor_notebook_controller: EditorNotebookController,
    ):
        super().__init__()
        self._ssb_fm = ssb_fm
        self._absolute_path = absolute_path
        self._relative_path = absolute_path.replace(
            self._ssb_fm.project_fm.dir() + os.path.sep, ""
        )
        if self._absolute_path == self._relative_path:
            # You gotta love path handling!
            self._relative_path = absolute_path.replace(
                self._ssb_fm.project_fm.dir().replace("\\", "/") + "/", ""
            )
    ...
    def on_ssb_property_change(self, loaded_ssb: SsbLoadedFile, name, value):
        ...
        self._ssbs_states[loaded_ssb.filename] = (
            not loaded_ssb.not_breakable,
            loaded_ssb.ram_state_up_to_date,
        )
    ...
    def load(
        ...
        def load_thread():
            try:
                ...
                exps_source, _ = self._ssb_fm.project_fm.explorerscript_load(
                    self._relative_path, sourcemap=False
                )

                # 2. Load a list of all ssbs file from the inclusion map,
                #    request them from the file manager and watch them
                inclusion_map_path = (
                    self._absolute_path + EXPLORERSCRIPT_INCLUSION_MAP_SUFFIX
                )
                inclusion_map = []
                ...
                for ssb_filename in inclusion_map:
                    ...
                    try:
                        self._register_ssb_handler(self._ssb_fm.get(ssb_filename))
                ...
                GLib.idle_add(partial(load_view_callback, exps_source, "exps"))
            ...
            for loaded_ssb in self._registered_ssbs:
                if self._is_breakable(loaded_ssb):
                    for opcode_offset, source_mapping in loaded_ssb.exps.source_map:
                        if isinstance(
                            source_mapping, MacroSourceMapping
                        ) and self._sm_entry_is_for_us(
                            ...
                            self._do_insert_opcode_text_mark(
                                ...
                                opcode_offset,
                                source_mapping.line,
                                source_mapping.column,
                                False,
                            )
                        # Also insert opcode text marks for macro calls
                        if (
                            isinstance(source_mapping, MacroSourceMapping)
                            and source_mapping.called_in
                            and source_mapping.called_in
                        ):
    ...
    def save(
        ...
        def save_thread():
            try:
                ready_to_reload_list, included_exps_files_list = (
                    self._ssb_fm.save_explorerscript_macro(
                ...
                GLib.idle_add(
                    partial(
                        self._after_save,
                        ready_to_reload_list,
                        included_exps_files_list,
                        success_callback,
                    )
                )
    ...
    def _after_save(
        ...
        zipped = zip(
            self._registered_ssbs, ready_to_reload_list, included_exps_files_list
        )
        ...
        for loaded_ssb, ready_to_reload, included_exps_files in zipped:
            for exps_abs_path in included_exps_files:
                ...
                self._editor_notebook_controller.on_exps_macro_ssb_changed(
                    exps_abs_path, loaded_ssb.filename
                )
            logger.debug(
                f"After save: Inform recompile ssb file {loaded_ssb.filename}."
            )
            self._editor_notebook_controller.on_ssb_changed_externally(
                loaded_ssb.filename, ready_to_reload
            )
        ...
        for loaded_ssb, ready_to_reload, included_exps_files in zipped:
            if ready_to_reload:
                logger.debug(
                    f"After save: MACRO - READY TO RELOAD NOW {loaded_ssb.filename}"
                )
    ...
    def on_ssb_changed_externally(self, ssb_filename, ready_to_reload):
        ...
        if loaded_ssb is not None:
            logger.error(
                f"SSB file {ssb_filename} for Macro {self.exps_filepath} was changed externally... Loading opcodes..."
            )
    ...
    def on_exps_macro_ssb_changed(self, exps_abs_path, ssb_filename):
        ...
        if exps_abs_path == self._absolute_path:
            if ssb_filename not in [ssb.filename for ssb in self._registered_ssbs]:
                logger.error(
                    f"SSB file {ssb_filename} new for Macro {self.exps_filepath}."
                )
                self._register_ssb_handler(self._ssb_fm.get(ssb_filename))
            if not self._we_triggered_the_reload:
                logger.error(
                    f"SSB file {ssb_filename} macro change treiggered for {self.exps_filepath}."
                )
    ...
    def _is_breakable(self, loaded_ssb: SsbLoadedFile):
        return (
            not loaded_ssb.not_breakable
            and self._ssb_fm.project_fm.explorerscript_hash_up_to_date(
                loaded_ssb.filename, loaded_ssb.exps.ssb_hash
            )
        )

    def _sm_entry_is_for_us(self, loaded_ssb: SsbLoadedFile, cmp_path: str | None):
        relpath_of_us_to_ssb_source = os.path.relpath(
            self._absolute_path, os.path.dirname(loaded_ssb.exps.full_path)
        )
