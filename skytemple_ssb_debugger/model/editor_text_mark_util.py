...
class EditorTextMarkUtil:
    ...
    @classmethod
    def get_line_marks_for(
        cls, b: GtkSource.Buffer, line: int, category: str
    ) -> list[GtkSource.Mark]:
        return b.get_source_marks_at_line(line, category)

    @classmethod
    def get_opcodes_in_line(
        cls, b: GtkSource.Buffer, line: int
    ) -> Iterable[tuple[str, int]]:
        return cls._get_opcode_in_line(b, line)

    @classmethod
    def extract_opcode_data_from_line_mark(
        cls, mark: GtkSource.Mark
    ) -> tuple[str, int]:
        name = mark.get_name()
    ...
    @classmethod
    def add_breakpoint_line_mark(
        cls, b: GtkSource.Buffer, ssb_filename: str, opcode_offset: int
    ):
        ms = []
    ...
    @classmethod
    def remove_breakpoint_line_mark(
        cls, b: GtkSource.Buffer, ssb_filename: str, opcode_offset: int
    ):
        ...
        for i in [0, 1]:
            ...
            if m is None:
                return
            b.remove_source_marks(
                b.get_iter_at_mark(m), b.get_iter_at_mark(m), CATEGORY_BREAKPOINT
            )

    @classmethod
    def create_opcode_mark(
        cls,
        b: GtkSource.Buffer,
        ssb_filename: str,
        offset: int,
        ...
        b.create_source_mark(
            f"opcode_<<<{ssb_filename}>>>_{offset}{macro_call_suffix}",
            CATEGORY_OPCODE,
            textiter,
        )

    @classmethod
    def _get_opcode_mark(
        cls,
        b: GtkSource.Buffer,
        ssb_filename: str,
        opcode_addr: int,
    ...
    @classmethod
    def _get_opcode_in_line(
        cls, buffer: GtkSource.Buffer, line
    ) -> list[tuple[str, int]]:
