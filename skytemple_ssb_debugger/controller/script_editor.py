...
class ScriptEditorController:
    def __init__(
        self,
        parent: EditorNotebookController,
        ...
        self._lm.set_search_path(
            self._lm.get_search_path() + [os.path.join(path, "..")]
        )
        ...
        self._hanger_halt_lines_after_load: (
            tuple[str, list[tuple[SsbRoutineType, int, int]]] | None
        ) = None
        ...
        self._mrk_attrs__breakpoint: GtkSource.MarkAttributes = (
            GtkSource.MarkAttributes.new()
        )
        self._mrk_attrs__breakpoint.set_pixbuf(create_breakpoint_icon())

        self._mrk_attrs__breaked_line: GtkSource.MarkAttributes = (
            GtkSource.MarkAttributes.new()
        )

        self._mrk_attrs__execution_line: GtkSource.MarkAttributes = (
            GtkSource.MarkAttributes.new()
        )
        ...
        self.file_context.register_insert_opcode_text_mark_handler(
            self.insert_opcode_text_mark
        )
        self.file_context.register_clear_opcode_text_mark_handler(
            self.clear_opcode_text_marks
        )
        ...
        builder_get_assert(
            self.builder, Gtk.ToggleToolButton, "code_editor_cntrls_breaks"
        ).set_active(self.parent.parent.global_state__breaks_disabled)
    ...
    def has_changes(self):
        if (
            ...
            and self._explorerscript_view.get_buffer().get_modified()
        ):
            return True
        return False
    ...
    def toggle_debugging_controls(self, val):
        builder_get_assert(
            self.builder, Gtk.ToolButton, "code_editor_cntrls_resume"
        ).set_sensitive(val)
        builder_get_assert(
            self.builder, Gtk.ToolButton, "code_editor_cntrls_step_over"
        ).set_sensitive(val)
        builder_get_assert(
            self.builder, Gtk.ToolButton, "code_editor_cntrls_step_into"
        ).set_sensitive(val)
        builder_get_assert(
            self.builder, Gtk.ToolButton, "code_editor_cntrls_step_out"
        ).set_sensitive(val)
        builder_get_assert(
            self.builder, Gtk.ToolButton, "code_editor_cntrls_step_next"
        ).set_sensitive(val)
    ...
    def on_break_pulled(self, ssb_filename, opcode_addr, halted_on_call):
        ...
        if self._still_loading:
            self._on_break_pulled_after_load = (
                ssb_filename,
                opcode_addr,
                halted_on_call,
            )
    ...
    def on_ssb_changed_externally(self, ssb_filename, ready_to_reload):
        ...
        """
        self.file_context.on_ssb_changed_externally(
            ssb_filename, ready_to_reload and not self.has_changes
        )
    ...
    def insert_hanger_halt_lines(
        self, ssb_filename: str, lines: list[tuple[SsbRoutineType, int, int]]
    ):
        ...
        if self._still_loading:
            ...
            for type, slot_id, opcode_addr in lines:
                EditorTextMarkUtil.add_line_mark_for_op(
                    ...
                    ssb_filename,
                    opcode_addr,
                    f"execution_{type.value}_{type.value}_{slot_id}",
                    "execution-line",
    ...
    def save(self):
        ...
        if modified_buffer.get_line_count() > 4000 or isinstance(
            self.file_context, ExpsMacroFileScriptFileContext
        ):
    ...
    def _save_done(self, modified_buffer: GtkSource.Buffer):
        ...
        for line in range(0, modified_buffer.get_line_count()):
            ...
            if len(marks) > 0:
                for (
                    ssb_filename,
                    opcode_offset,
                ) in EditorTextMarkUtil.get_opcodes_in_line(modified_buffer, line):
        ...
        for ssb_filename, b_points in breakpoints_to_resync.items():
            ...
            emulator_debug_breakpoints_resync(
                ssb_filename, b_points, self.parent.file_manager.get(ssb_filename)
            )
    ...
    def load_views(self, exps_bx: Gtk.Box):
        ...
        (
            exps_ovl,
            self._explorerscript_view,
            self._explorerscript_revealer,
            self._explorerscript_search,
            self._explorerscript_search_context,
        ) = self._create_editor()
        ...
        def load__gtk__process_loaded(text, language):
            ...
            undo_manager: GtkSource.UndoManager = assert_not_none(
                buffer.get_undo_manager()
            )
        ...
        def load__gtk__exps_exception(exc_info, exception):
            self.parent.get_context().display_error(
                ...
                f(
                    _(
                        "There was an error while loading the ExplorerScript "
    ...
    def remove_breakpoint(self, mark: GtkSource.Mark):
        ssb_filename, opcode_offset = (
            EditorTextMarkUtil.extract_opcode_data_from_line_mark(mark)
        )
    ...
    def on_breakpoint_removed(self, ssb_filename, opcode_offset):
        ...
        EditorTextMarkUtil.remove_breakpoint_line_mark(
            buffer, ssb_filename, opcode_offset
        )
    ...
    def insert_opcode_text_mark(
        self,
        ssb_filename: str,
        opcode_offset: int,
        line: int,
        column: int,
        is_for_macro_call=False,
    ):
        view = self._explorerscript_view
    ...
    def on_ssbs_state_change(self, breakable: bool, _ram_state_up_to_date: bool):
        ...
        info_bar = builder_get_assert(
            self.builder, Gtk.InfoBar, "code_editor_box_es_bar"
        )
        ...
        if not breakable:
            self._refill_info_bar(
                ...
                _(
                    "An old version of this script is still loaded in RAM. Debugging is disabled for this file, until it is reloaded."
                ),
            )
            return
    ...
    def on_sourceview_line_mark_activated(
        self, widget: GtkSource.View, textiter: Gtk.TextIter, event: Gdk.Event
    ):
    ...
    def on_sourcebuffer_delete_range(
        self, buffer: GtkSource.Buffer, start: Gtk.TextIter, end: Gtk.TextIter
    ):
    ...
    def on_search_entry_search_changed(self, widget: Gtk.SearchEntry):
        ...
        found, match_start, match_end = context.forward(
            buffer.get_iter_at_offset(buffer.props.cursor_position)
        )[:3]
        if found:
    ...
    def on_search_up_button_clicked(self, widget: Gtk.Button, search: Gtk.SearchEntry):
        ...
        found, match_start, match_end, wrap = context.backward(
            buffer.get_iter_at_offset(buffer.props.cursor_position)
        )[:4]
        if found:
            buffer.select_range(match_start, match_end)
    ...
    def on_search_down_button_clicked(
        self, widget: Gtk.Button, search: Gtk.SearchEntry
    ):
        view = self._explorerscript_view
        context = self._explorerscript_search_context
        buffer: Gtk.TextBuffer = view.get_buffer()
    ...
    def on_sr_search_clicked(self, btn: Gtk.Button, *args):
        ...
        settings.set_search_text(
            builder_get_assert(self.builder, Gtk.Entry, "sr_search_text").get_text()
        )
        cursor = buffer.get_iter_at_offset(buffer.props.cursor_position)
        search_down = not builder_get_assert(
            self.builder, Gtk.CheckButton, "sr_search_setting_search_backwards"
        ).get_active()
        if search_down:
            found, match_start, match_end, wrap = self._active_search_context.forward(
                cursor
            )
        else:
            found, match_start, match_end, wrap = self._active_search_context.backward(
                cursor
            )
        if found:
            if search_down and match_start.get_offset() == cursor.get_offset():
                # Repeat once, to really get down
                found, match_start, match_end, wrap = (
                    self._active_search_context.forward(match_end)
                )
            buffer.select_range(match_start, match_end)
            self._explorerscript_view.scroll_to_iter(match_start, 0.1, False, 0.5, 0.5)
    ...
    def on_sr_replace_clicked(self, btn: Gtk.Button, *args):
        ...
        if found:
            ...
            self._active_search_context.replace(
                match_start,
                match_end,
                builder_get_assert(
                    self.builder, Gtk.Entry, "sr_replace_text"
                ).get_text(),
                -1,
            )
    ...
    def on_sr_replace_all_clicked(self, btn: Gtk.Button, *args):
        ...
        self._active_search_context.replace_all(
            builder_get_assert(self.builder, Gtk.Entry, "sr_replace_text").get_text(),
            -1,
        )
    ...
    def menu__cut(self):
        ...
        b.cut_clipboard(
            Gtk.Clipboard.get(Gdk.Atom.intern("CLIPBOARD", False)), v.get_editable()
        )
    ...
    def menu__replace(self):
        ...
        if not self._loaded_search_window:
            ...
            search_settings: GtkSource.SearchSettings = (
                self._active_search_context.get_settings()
            )
            self._loaded_search_window = builder_get_assert(
                self.builder, Gtk.Dialog, "sr_dialog"
            )
            builder_get_assert(
                self.builder, Gtk.CheckButton, "sr_search_setting_case_sensitive"
            ).set_active(search_settings.get_case_sensitive())
            builder_get_assert(
                self.builder, Gtk.CheckButton, "sr_search_setting_match_words"
            ).set_active(search_settings.get_at_word_boundaries())
            builder_get_assert(
                self.builder, Gtk.CheckButton, "sr_search_setting_regex"
            ).set_active(search_settings.get_regex_enabled())
            builder_get_assert(
                self.builder, Gtk.CheckButton, "sr_search_setting_wrap_around"
            ).set_active(search_settings.get_wrap_around())
            self._loaded_search_window.set_title(
                f(_("Search and Replace in {self.filename}"))
            )
    ...
    def switch_style_scheme(self, scheme):
        ...
        self._mrk_attrs__execution_line.set_background(
            self._mix_breakpoint_colors("def:note", 21, 234, "#6D5900")
        )
        self._mrk_attrs__breaked_line.set_background(
            self._mix_breakpoint_colors("def:note", 81, 174, "#6D5900")
        )
        self._mrk_attrs__breakpoint.set_background(
            self._mix_breakpoint_colors("def:error", 51, 204, "#6D0D00")
        )

    # Utility
    def _mix_breakpoint_colors(
        self, mix_style_name, mix_style_alpha, text_style_alpha, fallback_color
    ):
        ...
        try:
            background_mix_style = assert_not_none(
                self._active_scheme.get_style(mix_style_name)
            ).props.background
        ...
        try:
            text_mix_bg_style = assert_not_none(
                self._active_scheme.get_style("text")
            ).props.background
        except (AttributeError, AssertionError):
            pass
        ...
        return Gdk.RGBA(*get_mixed_color(breakpoint_bg, text_bg))
    ...
    def _create_editor(
        ...
        up_button: Gtk.Button = Gtk.Button.new_from_icon_name(
            "skytemple-go-up-symbolic", Gtk.IconSize.BUTTON
        )
        up_button.set_can_focus(False)
        down_button: Gtk.Button = Gtk.Button.new_from_icon_name(
            "skytemple-go-down-symbolic", Gtk.IconSize.BUTTON
        )
    ...
    @staticmethod
    def _refill_info_bar(
        info_bar: Gtk.InfoBar, message_type: Gtk.MessageType, text: str
    ):
...
def get_mixed_color(color_rgba1, color_rgba2):
    red = (
        color_rgba1[0] * (255 - color_rgba2[3]) + color_rgba2[0] * color_rgba2[3]
    ) / 255
    green = (
        color_rgba1[1] * (255 - color_rgba2[3]) + color_rgba2[1] * color_rgba2[3]
    ) / 255
    blue = (
        color_rgba1[2] * (255 - color_rgba2[3]) + color_rgba2[2] * color_rgba2[3]
    ) / 255
...
class PlayIconRenderer(GtkSource.GutterRendererPixbuf):
    ...
    def __init__(self, view, **properties):
        ...
        self.empty = Gdk.pixbuf_get_from_surface(
            cairo.ImageSurface(cairo.FORMAT_ARGB32, 1, 1), 0, 0, 1, 1
        )
        self.set_size(12 * 3)
        self.set_padding(5, 3)
        icon_theme: Gtk.IconTheme = Gtk.IconTheme.get_for_screen(self.view.get_screen())
        self._icon_actor = icon_theme.load_icon(
            ICON_ACTOR[:-9] + "-gutter", 12, Gtk.IconLookupFlags.FORCE_SIZE
        ).copy()  # type: ignore
        self._icon_object = icon_theme.load_icon(
            ICON_OBJECT[:-9] + "-gutter", 12, Gtk.IconLookupFlags.FORCE_SIZE
        ).copy()  # type: ignore
        self._icon_performer = icon_theme.load_icon(
            ICON_PERFORMER[:-9] + "-gutter", 12, Gtk.IconLookupFlags.FORCE_SIZE
        ).copy()  # type: ignore
        self._icon_global_script = icon_theme.load_icon(
            ICON_GLOBAL_SCRIPT[:-9] + "-gutter", 12, Gtk.IconLookupFlags.FORCE_SIZE
        ).copy()  # type: ignore
        ...
        if len(breaked_marks) > 0:
            ...
            if len(execution_marks) > 0:
                _, type_id, slot_id = EXECUTION_LINE_PATTERN.match(
                    execution_marks[0].get_name()
                ).groups()  # type: ignore
            self.set_pixbuf(
                create_breaked_line_icon(
                    int(type_id),
                    int(slot_id),
                    self._icon_actor,
                    self._icon_object,
