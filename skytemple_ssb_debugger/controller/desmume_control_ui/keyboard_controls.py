...
class KeyboardControlsDialogController:
    ...
    def run(self, keyboard_cfg: list[int]) -> list[int] | None:
        ...
        for i in range(0, EmulatorKeys.NB_KEYS):
            ...
            b.set_label(
                f"{key_names_localized[i]} : {Gdk.keyval_name(self._keyboard_cfg[i])}"
            )
    ...
    def on_button_kb_key_clicked(self, w, *args):
        ...
        if dlg.run() == Gtk.ResponseType.OK:
            ...
            builder_get_assert(
                self.builder, Gtk.Button, f"button_{emulator_get_key_names()[key]}"
            ).set_label(
                f"{key_names_localized[key]} : {Gdk.keyval_name(self._tmp_key)}"
            )
