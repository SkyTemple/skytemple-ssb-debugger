...
class GroundStateController:
    def __init__(self, debugger_controller: DebuggerController, builder: Gtk.Builder):
        ...
        self._files__sw = builder_get_assert(
            builder, Gtk.ScrolledWindow, "ground_state_files_tree_sw"
        )
        self._entities__sw = builder_get_assert(
            builder, Gtk.ScrolledWindow, "ground_state_entities_tree_sw"
        )

        self._files__not_loaded = builder_get_assert(
            builder, Gtk.Viewport, "ground_state_files_tree_engine_not_loaded"
        )
        self._entities__not_loaded = builder_get_assert(
            builder, Gtk.Viewport, "ground_state_entities_tree_engine_not_loaded"
        )

        self._ssb_tree_store_iters: dict[int, Gtk.TreeIter] = {}

        self._files__tree = builder_get_assert(
            builder, Gtk.TreeView, "ground_state_files_tree"
        )
        ...
        self._files__tree.append_column(
            resizable(
                create_tree_view_column(_("Hanger"), Gtk.CellRendererText(), text=3)
            )
        )  # TRANSLATOR: Special context of a loaded script... the weird is engrish.
        self._files__tree.append_column(
            resizable(
        ...
        self._entities__tree = builder_get_assert(
            builder, Gtk.TreeView, "ground_state_entities_tree"
        )
        ...
        self._entities__tree.append_column(
            resizable(
                create_tree_view_column(_("Kind"), Gtk.CellRendererText(), text=5)
            )
        )
        self._entities__tree.append_column(
            resizable(
                create_tree_view_column(_("Hanger"), Gtk.CellRendererText(), text=1)
            )
        )  # TRANSLATOR: Special context of a loaded script... the weird is engris
        self._entities__tree.append_column(
            resizable(create_tree_view_column(_("X"), Gtk.CellRendererText(), text=8))
        )
        self._entities__tree.append_column(
            resizable(create_tree_view_column(_("Y"), Gtk.CellRendererText(), text=9))
        )
        self._entities__tree.append_column(
            resizable(
                create_tree_view_column(_("Sector"), Gtk.CellRendererText(), text=2)
            )
        )  # TRANSLATOR: Engrish for 'Layer'
        self._entities__tree.append_column(
            resizable(
                create_tree_view_column(_("Script"), Gtk.CellRendererText(), text=3)
            )
        )

        self._files__tree_store = builder_get_assert(
            builder, Gtk.TreeStore, "ground_state_files_tree_store"
        )
        self._entities__tree_store = builder_get_assert(
            builder, Gtk.TreeStore, "ground_state_entities_store"
        )
        ...
        if ground_state is not None:
            for ssb in ground_state.loaded_ssb_files:
                if ssb is not None and ssb.hanger in self._ssb_tree_store_iters:
                    if ssb.breaked:
                        self._entities__tree_store[
                            self._ssb_tree_store_iters[ssb.hanger]
                        ][6] = "media-playback-pause"
                    else:
                        self._entities__tree_store[
                            self._ssb_tree_store_iters[ssb.hanger]
                        ][6] = ""
    ...
    def sync(self, code_editor=None, breaked_for: ScriptRuntimeStruct | None = None):
        ...
        if self.debugger and self.debugger.ground_engine_state:
            ...
            if ground_state.running:
                # Is runningues
                global_script, ssb, ssx, actors, objects, performers, events, themap = (
                    ground_state.collect()
                )
                ...
                if code_editor:
                    ...
                    try:
                        if (
                            global_script.script_struct.hanger_ssb > -1
                            and ssb[global_script.script_struct.hanger_ssb] is not None
                        ):
                            if (
                                ssb[global_script.script_struct.hanger_ssb].file_name
                                not in files
                            ):
                                files[
                                    ssb[
                                        global_script.script_struct.hanger_ssb
                                    ].file_name
                                ] = []
                            files[
                                ssb[global_script.script_struct.hanger_ssb].file_name
                                ...
                                (
                                    SsbRoutineType.GENERIC,
                                    0,
                                    global_script.script_struct.current_opcode_addr_relative,
                                )
                            )
                    except IndexError:
                        pass
                    for i, actor in enumerate(actors):
                        try:
                            if (
                                actor is not None
                                and actor.script_struct.hanger_ssb > -1
                            ):
                                if ssb[actor.script_struct.hanger_ssb]:
                                    if (
                                        ssb[actor.script_struct.hanger_ssb].file_name
                                        not in files
                                    ):
                                        files[
                                            ssb[
                                                actor.script_struct.hanger_ssb
                                            ].file_name
                                        ] = []
                                    files[
                                        ssb[actor.script_struct.hanger_ssb].file_name
                                        ...
                                        (
                                            ...
                                            i,
                                            actor.script_struct.current_opcode_addr_relative,
                                        )
                                    )
                        except IndexError:
                            pass
                    for i, object in enumerate(objects):
                        try:
                            if (
                                object is not None
                                and object.script_struct.hanger_ssb > -1
                            ):
                                if ssb[object.script_struct.hanger_ssb]:
                                    if (
                                        ssb[object.script_struct.hanger_ssb].file_name
                                        not in files
                                    ):
                                        files[
                                            ssb[
                                                object.script_struct.hanger_ssb
                                            ].file_name
                                        ] = []
                                    files[
                                        ssb[object.script_struct.hanger_ssb].file_name
                                        ...
                                        (
                                            SsbRoutineType.OBJECT,
                                            i,
                                            object.script_struct.current_opcode_addr_relative,
                                        )
                                    )
                        except IndexError:
                            pass
                    for i, performer in enumerate(performers):
                        try:
                            if (
                                ...
                                and performer.script_struct.hanger_ssb > -1
                            ):
                                if ssb[performer.script_struct.hanger_ssb]:
                                    if (
                                        ssb[
                                            performer.script_struct.hanger_ssb
                                        ].file_name
                                        not in files
                                    ):
                                        files[
                                    ...
                                    files[
                                        ...
                                        (
                                            SsbRoutineType.PERFORMER,
                                            i,
                                            performer.script_struct.current_opcode_addr_relative,
                                        )
                                    )
                        except IndexError:
                            pass
                ...
                if ssb[0]:
                    self._files__tree_store.append(
                        None,
                        [
                            "skytemple-e-script-symbolic",
                            self.short_fname(ssb[0].file_name),
                    ...
                    self._files__tree_store.append(
                        None,
                        [
                            "skytemple-action-unavailable-symbolic",
                            _("<Empty>"),
                            "",
                            _("0 (Global)"),
                        ],
                ...
                for i in range(1, 4):
                    ...
                    if ssx[i]:
                        # Slot is filled
                        ssx_root = self._files__tree_store.append(
                            None,
                            [
                                "skytemple-e-ground-symbolic",
                                self.short_fname(ssx[i].file_name),
                                type_str,
                                hanger_str,
                            ],
                        )
                    ...
                    if ssb[i]:
                        ...
                        self._files__tree_store.append(
                            ...
                            [
                                "skytemple-e-script-symbolic",
                                self.short_fname(ssb[i].file_name),
                                GE_FILE_STORE_SCRIPT,
                                hanger_str,
                            ],
                        )
                    if ssb[i + TALK_HANGER_OFFSET]:
                        ...
                        self._files__tree_store.append(
                            ...
                            [
                                "skytemple-e-script-symbolic",
                                self.short_fname(ssb[i + TALK_HANGER_OFFSET].file_name),
                                GE_FILE_STORE_SCRIPT,
                                f"{i + TALK_HANGER_OFFSET} (Talk)",
                            ],
                        )
                ...
                try:
                    if (
                        ...
                        breaked = (
                            ssb[global_script.script_struct.hanger_ssb].breaked
                            and global_script.script_struct == breaked_for
                        )
                except (IndexError, AttributeError):
                    pass
                self._entities__tree_store.append(
                    None,
                    [
                        _("<Global>"),
                        "0",
                        "",
                        self.get_short_sname(
                            ssb, breaked, global_script.script_struct.hanger_ssb
                        ),
                        None,
                        "",
                        "skytemple-media-playback-pause-symbolic" if breaked else "",
                        ICON_GLOBAL_SCRIPT,
                        f"{themap.camera_x_pos}",
                        f"{themap.camera_y_pos}",
                        SsbRoutineType.GENERIC.value,
                    ],
                )
                actors_node = self._entities__tree_store.append(
                    None,
                ...
                for actor in actors:
                    ...
                    try:
                        if actor.script_struct.hanger_ssb > -1:
                            breaked = (
                                ssb[actor.script_struct.hanger_ssb].breaked
                                and actor.script_struct == breaked_for
                            )
                    except (IndexError, AttributeError):
                        pass
                    self._entities__tree_store.append(
                        ...
                        [
                            ...
                            f"{actor.sector}",
                            self.get_short_sname(
                                ssb, breaked, actor.script_struct.hanger_ssb
                            ),
                            None,
                            f"{actor.kind.name}",
                            "skytemple-media-playback-pause-symbolic"
                            if breaked
                            else "",
                            "",
                            f"{actor.x_map}",
                            f"{actor.y_map}",
                            SsbRoutineType.ACTOR.value,
                        ],
                    )
                objects_node = self._entities__tree_store.append(
                    None,
                ...
                for object in objects:
                    ...
                    try:
                        if object.script_struct.hanger_ssb > -1:
                            breaked = (
                                ssb[object.script_struct.hanger_ssb].breaked
                                and object.script_struct == breaked_for
                            )
                    except (IndexError, AttributeError):
                        pass
                    self._entities__tree_store.append(
                        ...
                        [
                            ...
                            f"{object.sector}",
                            self.get_short_sname(
                                ssb, breaked, object.script_struct.hanger_ssb
                            ),
                            None,
                            kind_name,
                            "skytemple-media-playback-pause-symbolic"
                            ...
                            f"{object.x_map}",
                            f"{object.y_map}",
                            SsbRoutineType.OBJECT.value,
                        ],
                    )
                performers_node = self._entities__tree_store.append(
                    None,
                    [
                        _("Performers"),
                        "",
                ...
                for performer in performers:
                    ...
                    try:
                        if performer.script_struct.hanger_ssb > -1:
                            breaked = (
                                ssb[performer.script_struct.hanger_ssb].breaked
                                and performer.script_struct == breaked_for
                            )
                    except (IndexError, AttributeError):
                        pass
                    self._entities__tree_store.append(
                        ...
                        [
                            ...
                            f"{performer.sector}",
                            self.get_short_sname(
                                ssb, breaked, performer.script_struct.hanger_ssb
                            ),
                            None,
                            f"{performer.kind}",
                            "skytemple-media-playback-pause-symbolic"
                            ...
                            f"{performer.x_map}",
                            f"{performer.y_map}",
                            SsbRoutineType.PERFORMER.value,
                        ],
                    )
                events_node = self._entities__tree_store.append(
                    None,
                ...
                pos_marks_node = self._entities__tree_store.append(
                    None,
                    [
                        _("Pos. Marks"),
                        "",
                            ...
                            for mark in marks:
                                self._entities__tree_store.append(
                                    ...
                                    [
                                        ...
                                        "",
                                        "",
                                        ssbf.file_name.split("/")[-1],
                                        None,
                                        "",
                                        "",
                                        "",
                                        f"{mark.x_with_offset}",
                                        f"{mark.y_with_offset}",
                                        -1,
                                    ],
                                )
    ...
    def get_short_sname(ssbs, breaked, hanger):
        ...
        try:
            if ssbs[hanger]:
                ...
                if (
                    breaked
                    and ssbs[hanger].breaked
                    and ssbs[hanger].breaked__handler_file != ssbs[hanger].file_name
                ):
