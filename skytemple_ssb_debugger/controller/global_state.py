...
class GlobalStateController:
    ...
    def sync(self):
        ...
        emulator_sync_tables(
            self.rom_data.bin_sections.itcm.data.MEMORY_ALLOCATION_TABLE.absolute_address,
            update,
        )

    def _apply_sync(self):
        store = builder_get_assert(
            self.builder, Gtk.ListStore, "global_state_alloc_store"
        )
        store.clear()
        if self._current_table < len(self._tables):
            ...
            for e in table.entries:
                ...
                store.append(line)
            builder_get_assert(
                self.builder, Gtk.Label, "lbl_alloc_table_header"
            ).set_text(
                f"{hex(table.start_address)} - {hex(table.start_address + 0x1c)}"
            )
            builder_get_assert(
                self.builder, Gtk.Label, "lbl_alloc_table_data"
            ).set_text(
                f"{hex(table.addr_data)} - {hex(table.addr_data + table.len_data)}"
            )
            builder_get_assert(
                self.builder, Gtk.Label, "lbl_alloc_table_entries"
            ).set_text(f"{len(table.entries)}/{table.max_entries}")
            parent = None
            for i, t in enumerate(self._tables):
                if table.parent_table == t.start_address:
                    parent = i
            builder_get_assert(
                self.builder, Gtk.Label, "lbl_alloc_table_parent"
            ).set_text(f"{hex(table.parent_table)} ({parent})")
        alloc_table_nb = builder_get_assert(
            self.builder, Gtk.SpinButton, "spin_alloc_table_nb"
        )
