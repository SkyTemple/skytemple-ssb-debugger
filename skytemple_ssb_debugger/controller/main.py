...
from skytemple_ssb_emulator import (
    ...
    emulator_get_jscfg,
    emulator_set_jscfg,
    emulator_keypad_add_key,
    emulator_keymask,
    EmulatorKeys,
    emulator_keypad_rm_key,
    emulator_touch_release,
    emulator_supports_joystick,
    emulator_is_running,
    SCREEN_HEIGHT_BOTH,
    emulator_volume_set,
...
from skytemple_ssb_debugger.controller.ground_state import (
    GroundStateController,
    GE_FILE_STORE_SCRIPT,
)
...
from skytemple_ssb_debugger.model.settings import (
    DebuggerSettingsStore,
    TEXTBOX_TOOL_URL,
)
...
class MainController:
    def __init__(
        ...
        builder: Gtk.Builder,
        window: Gtk.Window,
        control_context: AbstractDebuggerControlContext,
    ):
        ...
        spellcheck_enabled_item = builder_get_assert(
            self.builder, Gtk.CheckMenuItem, "menu_spellcheck_enabled"
        )
        ...
        for style_id in assert_not_none(self.style_scheme_manager.get_scheme_ids()):
            if not self.selected_style_scheme_id or (
                was_none_before and style_id == "oblivion"
            ):
                self.selected_style_scheme_id = style_id
            style_dict[style_id] = assert_not_none(
                self.style_scheme_manager.get_scheme(style_id)
            ).get_name()
        menu_view_schemes = builder_get_assert(
            self.builder, Gtk.MenuItem, "menu_view_schemes"
        )
        first_item = None
        ...
        for style_id, style_name in style_dict.items():
            ...
            new_item.set_property(
                "group", first_item
            )  # upstream bug; can't use the constructor for this.
            ...
            new_item.connect(
                "toggled", partial(self.on_menu_view_schemes_switch, style_id)
            )
        ...
        self._filter_gba_ds.set_name(
            _("Nintendo DS ROMs with binary loader (*.ds.gba)")
        )
        ...
        self.renderer = AsyncSoftwareRenderer(
            self.main_draw, self.sub_draw, self.debug_overlay.draw
        )
        self.renderer.start()
        ...
        if lang:
            ...
            if lang == Language.Japanese:
                builder_get_assert(
                    self.builder, Gtk.RadioMenuItem, "menu_emulator_language_jp"
                ).set_active(True)
            elif lang == Language.English:
                builder_get_assert(
                    self.builder, Gtk.RadioMenuItem, "menu_emulator_language_en"
                ).set_active(True)
            elif lang == Language.French:
                builder_get_assert(
                    self.builder, Gtk.RadioMenuItem, "menu_emulator_language_fr"
                ).set_active(True)
            elif lang == Language.German:
                builder_get_assert(
                    self.builder, Gtk.RadioMenuItem, "menu_emulator_language_de"
                ).set_active(True)
            elif lang == Language.Italian:
                builder_get_assert(
                    self.builder, Gtk.RadioMenuItem, "menu_emulator_language_it"
                ).set_active(True)
            elif lang == Language.Spanish:
                builder_get_assert(
                    self.builder, Gtk.RadioMenuItem, "menu_emulator_language_es"
                ).set_active(True)
            self._suppress_event = False
        ...
        self.variable_controller: VariableController = VariableController(
            self.builder, self.context
        )
        self.global_state_controller: GlobalStateController = GlobalStateController(
            self.builder
        )
        self.local_variable_controller: LocalVariableController = (
            LocalVariableController(self.builder, self.debugger)
        )
        self.ground_state_controller = GroundStateController(
            self.debugger, self.builder
        )

        # Load more initial settings
        self.on_debug_log_cntrl_ops_toggled(
            builder_get_assert(builder, Gtk.CheckButton, "debug_log_cntrl_ops")
        )
        self.on_debug_log_cntrl_script_toggled(
            builder_get_assert(builder, Gtk.CheckButton, "debug_log_cntrl_script")
        )
        self.on_debug_log_cntrl_internal_toggled(
            builder_get_assert(builder, Gtk.CheckButton, "debug_log_cntrl_internal")
        )
        self.on_debug_log_cntrl_ground_state_toggled(
            builder_get_assert(builder, Gtk.CheckButton, "debug_log_cntrl_ground_state")
        )
        self.on_debug_settings_debug_mode_toggled(
            builder_get_assert(builder, Gtk.CheckButton, "debug_settings_debug_mode")
        )
        self.on_debug_settings_overlay_toggled(
            builder_get_assert(builder, Gtk.CheckButton, "debug_settings_overlay")
        )
        self.on_emulator_controls_volume_toggled(
            builder_get_assert(builder, Gtk.ToggleButton, "emulator_controls_volume")
        )
        self.on_debug_log_scroll_to_bottom_toggled(
            builder_get_assert(builder, Gtk.ToggleButton, "debug_log_scroll_to_bottom")
        )
        ...
        # Other gtk stuff
        self._debug_log_textview_right_marker = (
            builder_get_assert(self.builder, Gtk.TextView, "debug_log_textview")
            .get_buffer()
            .create_mark(
                "end",
                builder_get_assert(self.builder, Gtk.TextView, "debug_log_textview")
        ...
        # Initial sizes
        builder_get_assert(self.builder, Gtk.Frame, "frame_debug_log").set_size_request(
            220, -1
        )
    ...
    def global_state__breaks_disabled(self):
        return builder_get_assert(
            self.builder, Gtk.CheckMenuItem, "menu_debugger_disable_breaks"
        ).get_active()
    ...
    def global_state__audio_enabled(self):
        return builder_get_assert(
            self.builder, Gtk.ToggleButton, "emulator_controls_volume"
        ).get_active()
    ...
    def init_emulator(self):
        try:
            ...
            self.context.display_error(
                ...
                f(
                    _(
                        "The emulator couldn't be loaded. "
    ...
    def install_poll_emulator(self):
        # 30 FPS
        self._poll_emulator_event_id = GLib.timeout_add(
            1000 // 30, self.do_poll_emulator
        )
    ...
    def do_poll_emulator(self):
        try:
            ...
            self.context.display_error(
                sys.exc_info(),
                f(
                    _("The emulator reported an internal error:\n\n")
    ...
    def on_main_window_key_press_event(
        ...
        if isinstance(self.window.get_focus(), Gtk.Entry) or isinstance(
            self.window.get_focus(), Gtk.TextView
        ):
            return False
    ...
    def on_main_window_key_release_event(
        self, widget: Gtk.Widget, event: Gdk.EventKey, *args
    ):
        key = self.lookup_key(event.keyval)
        if key and self.emu_is_running:
    ...
    def on_draw_main_draw(self, widget: Gtk.DrawingArea, ctx: cairo.Context, *args):
        if self.renderer:
            return self.renderer.screen(
                self._current_screen_width, self._current_screen_height, ctx, 0
            )
    ...
    def on_draw_sub_draw(self, widget: Gtk.DrawingArea, ctx: cairo.Context, *args):
        if self.renderer:
            return self.renderer.screen(
                self._current_screen_width, self._current_screen_height, ctx, 1
            )
    ...
    def on_draw_button_release_event(
    ...
    def on_draw_button_press_event(
        self, widget: Gtk.Widget, event: Gdk.EventButton, display_id: int
    ):
        widget.grab_focus()
        if event.button == 1:
    ...
    def on_menu_open_activate(self, *args):
        ...
        response, fn = self._file_chooser(
            Gtk.FileChooserAction.OPEN,
            _("Open..."),
            (self._filter_nds, self._filter_gba_ds, self._filter_any),
        )
        ...
        if response == Gtk.ResponseType.ACCEPT:
            try:
                ...
                self.context.display_error(
                    sys.exc_info(), f(_("Unable to load: {fn}\n{ex}"))
                )
            self.load_rom()
    ...
    def on_menu_view_schemes_switch(self, scheme_id: str, *args):
        if hasattr(self, "editor_notebook"):  # skip during __init__
            ...
            self.editor_notebook.switch_style_scheme(
                self.style_scheme_manager.get_scheme(scheme_id)
            )
    ...
    def on_menu_emulator_keyboard_controls_activate(
        self, button: Gtk.CheckMenuItem, *args
    ):
        new_keyboard_cfg = KeyboardControlsDialogController(self.window).run(
            self._keyboard_cfg
        )
    ...
    def on_menu_emulator_joystick_controls_activate(
        self, button: Gtk.CheckMenuItem, *args
    ):
    ...
    def on_menu_emulator_screenshot_activate(self, button: Gtk.CheckMenuItem, *args):
        ...
        if response == Gtk.ResponseType.ACCEPT:
            ...
            Image.frombuffer(
                "RGBA", (SCREEN_WIDTH, SCREEN_HEIGHT_BOTH), imgdata, "raw", "RGBA", 0, 1
            ).save(fn)

    # MENU HELP
    def on_menu_help_exps_docs_activate(self, btn: Gtk.MenuItem, *args):
        webbrowser.open_new_tab(
            "https://explorerscript.readthedocs.io/en/latest/language_spec.html"
        )

    def on_menu_help_wiki_activate(self, *args):
        webbrowser.open(
            f"{SKYTEMPLE_WIKI_LINK}/index.php/SkyTemple:UI-Link/skytemple-ssb-debugger"
        )
    ...
    def on_emulator_controls_playstop_clicked(self, button: Gtk.Button):
        if not self._stopped:
            ...
            if (
                not self.variable_controller.variables_changed_but_not_saved
                or self._warn_about_unsaved_vars()
            ):
                self.emu_reset()
                self.emu_resume()
    ...
    def on_emulator_controls_volume_toggled(self, button: Gtk.ToggleButton):
        ...
        self._suppress_event = True
        builder_get_assert(
            self.builder, Gtk.CheckMenuItem, "menu_emulator_volume"
        ).set_active(button.get_active())
        self._suppress_event = False
    ...
    def on_debug_log_clear_clicked(self, btn: Gtk.Button):
        buff = builder_get_assert(
            self.builder, Gtk.TextView, "debug_log_textview"
        ).get_buffer()
    ...
    def on_ssb_file_tree_button_press_event(
        ...
        if event.type == Gdk.EventType.DOUBLE_BUTTON_PRESS:
            ...
            if treeiter is not None and model is not None:
                if model[treeiter][0] == "":
                    ...
                    short_path = model[treeiter][0].replace(
                        self.context.get_project_dir() + os.path.sep, ""
                    )
                    self.editor_notebook.open_exps_macro(model[treeiter][0])
                else:
            ...
            if paths is not None:
                ...
                if treepath is not None:
                    if model[treepath][2] in [
                        ...
                        open_scene: Gtk.MenuItem = Gtk.MenuItem.new_with_label(
                            _("Open Scenes...")
                        )
                        open_scene.connect(
                            "activate",
                            lambda *args: self.context.open_scene_editor_for_map(
                                model[not_none(treepath)][0]
                            ),
                        )
                        menu.add(open_scene)
                        menu.show_all()
                    ...
                    if model[treepath][2] in ["map_sss_entry", "ssb"]:
                        ...
                        open_scene.connect(
                            "activate",
                            lambda *args: self.context.open_scene_editor(
                                self.get_scene_type_for(model[not_none(treepath)][0]),
                                self.get_scene_name_for(model[not_none(treepath)][0]),
                            ),
                        )
                        menu.add(open_scene)
                        menu.show_all()
                    ...
                    if model[treepath][2] == "exps_macro_dir":
                        menu = Gtk.Menu.new()
                        create_dir: Gtk.MenuItem = Gtk.MenuItem.new_with_label(
                            _("Create directory...")
                        )
                        create_dir.connect(
                            "activate",
                            partial(
                                self.on_ssb_file_tree__menu_create_macro_dir,
                                model.get_model(),
                                model.convert_path_to_child_path(treepath),
                            ),
                        )  # type: ignore
                        create_file: Gtk.MenuItem = Gtk.MenuItem.new_with_label(
                            _("Create new script file...")
                        )
                        create_file.connect(
                            "activate",
                            partial(
                                self.on_ssb_file_tree__menu_create_macro_file,
                                model.get_model(),
                                model.convert_path_to_child_path(treepath),
                            ),
                        )  # type: ignore
                        ...
                        if model[treepath][1] != _("Macros"):
                            ...
                            delete_dir: Gtk.MenuItem = Gtk.MenuItem.new_with_label(
                                _("Delete directory...")
                            )
                            delete_dir.connect(
                                "activate",
                                partial(
                                    self.on_ssb_file_tree__menu_delete_dir,
                                    model.get_model(),
                                    model.convert_path_to_child_path(treepath),
                                ),
                            )  # type: ignore
                        ...
                        menu = Gtk.Menu.new()
                        delete_file: Gtk.MenuItem = Gtk.MenuItem.new_with_label(
                            _("Delete script file...")
                        )
                        delete_file.connect(
                            "activate",
                            partial(
                                self.on_ssb_file_tree__menu_delete_file,
                                model.get_model(),
                                model.convert_path_to_child_path(treepath),
                            ),
                        )  # type: ignore
    ...
    def on_ssb_file_tree__menu_create_macro_dir(
        self, store: Gtk.TreeStore, treepath: Gtk.TreePath, *args
    ):
        row = store[treepath]
        response, dirname = self._show_generic_input(
            _("Name of the directory:"), _("Create Directory")
        )
        if response == Gtk.ResponseType.OK:
            abs_dirname = row[0] + os.path.sep + dirname
            os.makedirs(abs_dirname, exist_ok=True)
            store.append(
                store.get_iter(treepath), [abs_dirname, dirname, "exps_macro_dir", True]
            )

    def on_ssb_file_tree__menu_create_macro_file(
        self, store: Gtk.TreeStore, treepath: Gtk.TreePath, *args
    ):
        row = store[treepath]
        response, filename = self._show_generic_input(
            _("Name of the new script file:"), _("Create File")
        )
        ...
        if response == Gtk.ResponseType.OK:
            ...
            with open_utf8(abs_filename, "w") as f:
                f.write("")
            store.append(
                store.get_iter(treepath), [abs_filename, filename, "exps_macro", True]
            )

    def on_ssb_file_tree__menu_delete_dir(
        self, model: Gtk.TreeModel, treepath: Gtk.TreePath, *args
    ):
        row = model[treepath]
        response = self._show_are_you_sure_delete(
            f(
                _(
    ...
    def on_ssb_file_tree__menu_delete_file(self, model, treepath, *args):
        row = model[treepath]
        response = self._show_are_you_sure_delete(
            f(_("Do you want to delete the script file " "{row[1]}?"))
        )
        if response == Gtk.ResponseType.DELETE_EVENT:
            os.remove(row[0])
            del model[treepath]
    ...
    def init_file_tree(self):
        ...
        if not self._ssb_item_filter:
            ...
            builder_get_assert(self.builder, Gtk.TreeView, "ssb_file_tree").set_model(
                self._ssb_item_filter
            )
            assert self._ssb_item_filter is not None
        ...
        macros_tree_nodes = {
            macros_dir_name: ssb_file_tree_store.append(
        ...
        for root, dnames, fnames in os.walk(macros_dir_name):
            ...
            for dirname in dnames:
                macros_tree_nodes[root + os.path.sep + dirname] = (
                    ssb_file_tree_store.append(
                        root_node,
                        [root + os.path.sep + dirname, dirname, "exps_macro_dir", True],
                    )
                )
            for filename in fnames:
                if len(filename) > 4 and filename[-5:] == EXPLORERSCRIPT_EXT:
                    ssb_file_tree_store.append(
                        root_node,
                        [root + os.path.sep + filename, filename, "exps_macro", True],
                    )

        # SSB SCRIPT FILES
        #    -> Common [common]
        common_root = ssb_file_tree_store.append(
            None, ["", _("Common"), "common_dir", True]
        )
        #       -> Master Script (unionall) [ssb]
        #       -> (others) [ssb]
        for name in script_files["common"]:
            ssb_file_tree_store.append(
                common_root, ["COMMON/" + name, name, "ssb", True]
            )

        for i, map_obj in enumerate(script_files["maps"].values()):
            #    -> (Map Name) [map]
            map_root = ssb_file_tree_store.append(
                None, [map_obj["name"], map_obj["name"], "map_root", True]
            )
            self._registered_maps[map_obj["name"]] = map_root

            enter_root = ssb_file_tree_store.append(
                map_root, [map_obj["name"], _("Enter (sse)"), "map_sse", True]
            )
            ...
            #       -> Acting Scripts [lsd]
            acting_root = ssb_file_tree_store.append(
                map_root, [map_obj["name"], _("Acting (ssa)"), "map_ssa", True]
            )
            ...
            #       -> Sub Scripts [sub]
            sub_root = ssb_file_tree_store.append(
                map_root, [map_obj["name"], _("Sub (sss)"), "map_sss", True]
            )
            ...
            for sss, ssbs in map_obj["subscripts"].items():
                ...
                sub_entry = ssb_file_tree_store.append(
                    sub_root, [sss_name, sss, "map_sss_entry", True]
                )
    ...
    def on_global_state_alloc_dump_clicked(self, *args):
        active_rows: list[Gtk.TreePath] = (
            builder_get_assert(
                self.builder, Gtk.TreeView, "global_state_alloc_treeview"
            )
            .get_selection()
            .get_selected_rows()[1]
        )
    ...
    def on_ground_state_entities_tree_button_press_event(
        self, tree: Gtk.TreeView, event: Gdk.Event
    ):
        if event.type == Gdk.EventType.DOUBLE_BUTTON_PRESS:
            model, treeiter = tree.get_selection().get_selected()
            if treeiter is not None and model is not None:
                script_entity_type = SsbRoutineType.create_for_index(
                    model[treeiter][10]
                )
                script_entity_id = 0
                try:
                ...
                if ges:
                    ...
                    if not ssb:
                        return
                    self.editor_notebook.focus_by_opcode_addr(
                        ssb.file_name, ss.current_opcode_addr_relative
                    )

    def on_ground_state_files_tree_button_press_event(
        self, tree: Gtk.TreeView, event: Gdk.Event
    ):
        if event.type == Gdk.EventType.DOUBLE_BUTTON_PRESS:
            model, treeiter = tree.get_selection().get_selected()
    ...
    def load_rom(self):
        try:
            ...
            emulator_debug_init_breakpoint_manager(
                os.path.join(
                    self.context.get_project_debugger_dir(),
                    f"{os.path.basename(fn)}.breakpoints.json",
                )
            )
            ...
            if self.debugger:
                ...
                for i in range(0, 12):
                    fl1.append(
                        builder_get_assert(
                            self.builder, Gtk.CheckButton, f"chk_debug_flag_1_{i}"
                        ).get_active()
                    )
                for j in range(0, 16):
                    fl2.append(
                        builder_get_assert(
                            self.builder, Gtk.CheckButton, f"chk_debug_flag_2_{j}"
                        ).get_active()
                    )
                self.debugger.enable(
                    ...
                    debug_mode=builder_get_assert(
                        self.builder, Gtk.CheckButton, "debug_settings_debug_mode"
                    ).get_active(),
                    debug_flag_1=fl1,
                    debug_flag_2=fl2,
                )
    ...
    def enable_editing_features(self):
        ...
        code_editor_notebook = builder_get_assert(
            self.builder, Gtk.Notebook, "code_editor_notebook"
        )
        code_editor_main.remove(
            builder_get_assert(self.builder, Gtk.Widget, "main_label")
        )
    ...
    def savestate(self, i: int):
        ...
        try:
            ...
            desmume_savestate_path = os.path.join(
                self.context.get_project_debugger_dir(),
                f"{rom_basename}.save.{i}.{SAVESTATE_EXT_DESUME}",
            )
            ground_engine_savestate_path = os.path.join(
                self.context.get_project_debugger_dir(),
                f"{rom_basename}.save.{i}.{SAVESTATE_EXT_GROUND_ENGINE}",
            )
            ...
            self.context.display_error(
                sys.exc_info(), str(err), _("Unable to save savestate!")
            )
            return
    ...
    def loadstate(self, i: int):
        ...
        if os.path.exists(ground_engine_savestate_path):
            try:
                ...
                self.context.display_error(
                    sys.exc_info(), str(ex), _("Unable to load savestate!")
                )
                return
    ...
    def emu_reset(self):
        ...
        if self.debugger and self.debugger.ground_engine_state:
            ...
            debug_special_episode_number = (
                ramdata.DEBUG_SPECIAL_EPISODE_NUMBER.absolute_address
            )
    ...
    def on_script_added(self, ssb_path, mapname, scene_type, scene_name):
        ...
        ssb_file_tree_store.append(
            self._tree_branches[branch_name],
    ...
    def _create_tree_branch(self, mapname, branch):
        ...
        if mapname not in self._registered_maps:
            map_root = self._file_tree_store.append(
                None, [mapname, mapname, "map_root", True]
            )
            self._registered_maps[mapname] = map_root
        map_root = self._registered_maps[mapname]

        if branch == "enter":
            enter_root = self._file_tree_store.append(
                map_root, [mapname, _("Enter (sse)"), "map_sse", True]
            )
            self._tree_branches[f"{mapname}_enter"] = enter_root
        elif branch == "acting":
            acting_root = self._file_tree_store.append(
                map_root, [mapname, _("Acting (ssa)"), "map_ssa", True]
            )
            self._tree_branches[f"{mapname}_acting"] = acting_root
        else:
            if f"{mapname}_subroot" not in self._tree_branches:
                sub_root = self._file_tree_store.append(
                    map_root, [mapname, _("Sub (sss)"), "map_sss", True]
                )
            ...
            self._scene_types[sss_name] = "sss"
            self._scene_names[sss_name] = sss_name
            sub_entry = self._file_tree_store.append(
                sub_root, [sss_name, branch.replace("_", "/"), "map_sss_entry", True]
            )
            self._tree_branches[sss_name.replace("/", "_")] = sub_entry
    ...
    def on_chk_debug_flag_1_toggled(self, w):
        emulator_set_debug_flag_1(
            int(w.get_name()[len("chk_debug_flag_1_") :]), w.get_active()
        )

    def on_chk_debug_flag_2_toggled(self, w):
        emulator_set_debug_flag_2(
            int(w.get_name()[len("chk_debug_flag_2_") :]), w.get_active()
        )

    def set_check_debug_flag(self, var_id, flag_id, value):
        builder_get_assert(
            self.builder, Gtk.CheckButton, f"chk_debug_flag_{var_id}_{flag_id}"
    ...
    def break_pulled(self, state: BreakpointState):
        ...
        self.write_info_bar(
            Gtk.MessageType.WARNING, f(_("The debugger is halted at {ssb.file_name}."))
        )
    ...
    def step_into_macro_call(self, file_state: BreakpointFileState):
        ...
        self.editor_notebook.focus_by_opcode_addr(
            file_state.ssb_filename, file_state.opcode_addr
        )
        srs = ScriptRuntimeStruct.from_data(
            self.debugger.rom_data,
    ...
    def _filter__refresh_results(self):
        ...
        if self._search_text == "":
            ...
            builder_get_assert(
                self.builder, Gtk.TreeView, "ssb_file_tree"
            ).collapse_all()
    ...
    def _filter__expand_all_visible(self, model: Gtk.TreeStore, path, iter):
        ...
        if self._search_text:
            ...
            ssb_file_tree = builder_get_assert(
                self.builder, Gtk.TreeView, "ssb_file_tree"
            )
    ...
    def _file_chooser(self, type: Gtk.FileChooserAction, name, filter):
        dialog = Gtk.FileChooserNative.new(name, self.window, type, None, None)
        for f in filter:
            dialog.add_filter(f)
    ...
    def _debugger_print_callback(self, string):
        ...
        textview.get_buffer().insert(
            textview.get_buffer().get_end_iter(), string + "\n"
        )

        if self._debug_log_scroll_to_bottom:
            self._suppress_event = True
            textview.scroll_to_mark(
                self._debug_log_textview_right_marker, 0, True, 0, 0
            )
            self._suppress_event = False
    ...
    def _set_buttons_paused(self):
        btn: Gtk.Button = builder_get_assert(
            self.builder, Gtk.Button, "emulator_controls_pause"
        )
    ...
    def _show_are_you_sure_delete(self, text):
        ...
        dont_save: Gtk.Widget = dialog.add_button(
            _("Delete"), Gtk.ResponseType.DELETE_EVENT
        )
