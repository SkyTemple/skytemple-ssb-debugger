...
class VariableController:
    ...
    def sync(self):
        ...
        def update(vals: Mapping[int, Sequence[int]]):
            ...
            self._variable_cache = {
                self.rom_data.script_data.game_variables__by_id[k]: list(v)
    ...
    def _apply_sync(self):
        ...
        for i, sub in enumerate(self.var_form_elements):
            if sub is not None:
                for offset, el in enumerate(sub):
                    if el is not None:
                        val = self._variable_cache[
                            self.rom_data.script_data.game_variables__by_id[i]
                        ][offset]
        ...
        builder_get_assert(
            self.builder, Gtk.Notebook, "variables_notebook"
        ).set_sensitive(True)
        self._suppress_events = False
    ...
    def init(self, rom_data: Pmd2Data):
        ...
        for category, items in self.CATEGORIES.items():
            ...
            for item in items:
                ...
                if var.nbvalues == 1:
                    page_grid.attach(
                        self.create_var_form_element(var, 0, no_label=True),
                        0,
                        row + 1,
                        1,
                        1,
                    ...
                    var.name.startswith("SCENARIO_")
                    and var.nbvalues == 2
                    and var.type == GameVariableType.UINT8
                ):
                    ...
                    for i in range(0, var.nbvalues):
                        ...
                        y = math.floor(i / 3)
                        sub_grid.attach(
                            self.create_var_form_element(var, i), x, y, 1, 1
                        )
                else:
                    ...
                    for i in range(0, var.nbvalues):
                        sub_box.pack_start(
                            self.create_var_form_element(var, i), True, True, 0
                        )
                row += 2
    ...
    def create_var_form_element(
        self,
        var: Pmd2ScriptGameVar,
        offset: int,
        label: str | None = None,
        no_label=False,
    ):
        ...
        if var.type == GameVariableType.BIT:
            ...
            wgd2 = Gtk.Entry.new()
            if (
                var.type == GameVariableType.UINT32
                or var.type == GameVariableType.INT32
            ):
                wgd2.set_width_chars(11)
            elif (
                var.type == GameVariableType.UINT16
                or var.type == GameVariableType.INT16
            ):
                wgd2.set_width_chars(6)
            else:
                wgd2.set_width_chars(4)
            wgd2.connect(
                "focus-out-event", partial(self.on_var_changed_entry, var, offset)
            )
            wdg = wgd2
    ...
    def on_var_changed_entry(
        self, var: Pmd2ScriptGameVar, offset: int, wdg: Gtk.Entry, *args
    ):
        ...
        try:
            try:
                ...
                raise ValueError(
                    _("The variable must have a number as value.")
                ) from err
            if var.type == GameVariableType.BIT:
    ...
    def on_var_changed_check(
        self, var: Pmd2ScriptGameVar, offset: int, wdg: Gtk.CheckButton, *args
    ):
        if self._suppress_events:
            return
        self.variables_changed_but_not_saved = True
    ...
    def load(self, index: int, config_dir: str):
        ...
        try:
            ...
            self.context.display_error(
                sys.exc_info(), str(err), _("Unable to load variables!")
            )
            return
